...
function commandParserFactory(Logger, Promise, _) {
    ...
    CommandParser.prototype[dpkg] = function(data) {
        var dMap = {
            u: 'Unknown',
            i: 'Install',
            r: 'Remove',
        ...
        var eMap = {
            ...
            x: 'Hold & Resinst-required'
        };

        if (data.error) {
            return Promise.resolve({ source: 'dpkg', error: data.error });
        } else if (!data.stdout) {
            return Promise.resolve({ source: 'dpkg', error: new Error("No data") });
        }
        try {
            var refLine = data.stdout.split('\n')[3],
                nameMark = refLine.indexOf('Name'),
                versionMark = refLine.indexOf('Version'),
                arcMark = refLine.indexOf('Architecture'),
                descMark = refLine.indexOf('Description');

            var lines = data.stdout.split('\n').slice(5, -1);
            var parsed = _.map(lines, function(line) {
                var statColumns = line.slice(0,4),
                    name = line.substring(nameMark, versionMark),
                    version = line.substring(versionMark, arcMark),
                    architecture = line.substring(arcMark, descMark),
                    description = line.substring(descMark);
                return {
                    Desired: dMap[statColumns[0]],
                    Status: sMap[statColumns[1]],
                    Error: eMap[statColumns[2]],
                    Name: name.trim(),
            ...
            return Promise.resolve({ data: parsed, source: 'dpkg', store: true});
        } catch (e) {
            return Promise.resolve({ source: 'dpkg', error: e});
        }
    };
